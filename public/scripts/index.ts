...
export function updatePage(position: GeolocationPosition) {
  getWeatherAndInsertCurrentWeather(
...
function insertCurrentWeather(data: CurrentWeatherData): undefined {
  ...
  const metersPerSecToMph = (metersPerSec: number) =>
    Math.round(metersPerSec * 2.237);
  ...
  if (mainWeatherConditions === "Clear")
    ...
    mainWeatherConditions.toLowerCase() ===
  ...
  const emojiID =
    data.weather[0].id === 800 ? 800 : firstDigit(data.weather[0].id);

  let content = `
        <div class="mt-4 p-3 mainWeatherBackground text-black rounded border">
            <h1>It's currently ${currentFahrenheit}&deg; in ${city} ${emojiMap.get(
    emojiID
  )}</h1>
            <br/>
...
function renderRainPercentageChart(hourlyData: HourlyWeatherArray) {
  ...
  if (typeof Chart === "undefined") {
    console.error(
      "Chart.js not found. Make sure it is imported or available globally."
    );
    return;
  }
...
function createFutureWeatherContainers(data: DailyWeatherData[]) {
  ...
  if (!mainContainer) {
    console.error(
      'Document element with ID "futureWeatherContainer" not found'
    );
    return;
...
function insertFutureWeather(
  data: HourlyWeatherArray,
  dataDaily: DailyWeatherData[],
  sunsetTime: Date
) {
  ...
  data.forEach((forecast) => {
    ...
    var fahrenheitFeelLike = Math.round(
      (forecast.feels_like - 273.15) * 1.8 + 32
    );
...
function insertWeatherAlerts(alertsData: WeatherAlertArray) {
  ...
  if (warningContainer) {
    ...
    const collaspedAlertContainer = document.getElementById(
      "collaspedAlertContainer"
    );
